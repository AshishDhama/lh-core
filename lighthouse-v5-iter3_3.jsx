...
export default function LighthouseScrolly() {
  ...
  const tealBg = dk ? "rgba(0,133,117,.14)" : "rgba(0,133,117,.08)";
  ...
  const bd = {editorial:dk?"rgba(255,255,255,.05)":"rgba(0,0,0,.08)",notion:dk?"rgba(255,255,255,.06)":"rgba(0,0,0,.06)",m3:dk?"rgba(255,255,255,.08)":"rgba(0,0,0,.03)"}[designMode]||(dk?"rgba(255,255,255,.07)":"rgba(0,44,119,.07)");
  ...
  const Btn = ({children, primary, secondary, small, disabled, onClick, style:s}) => (
    <button onClick={disabled?undefined:onClick} disabled={disabled} aria-disabled={disabled||undefined}
      style={{padding:small?"7px 14px":"12px 24px",minHeight:small?32:44,borderRadius:br,
        ...
        background:disabled?"transparent":primary?navy:secondary?teal:isM?(dk?"rgba(255,255,255,.04)":"rgba(0,44,119,.04)"):isN?(dk?"rgba(255,255,255,.04)":"#F3F3F3"):"transparent",
  ...
  const DashboardPage = () => (
    <div className="an" style={{maxWidth:isE?780:720,margin:"0 auto",padding:isE?"40px 0 60px":"32px 0"}}>
      {/* Hero — editorial gets large type + greeting line, notion gets clean flat */}
      {isE ? <>
    ...
    return (
      <div className="an" style={{padding:"0"}}>
        ...
        <div className="an2" style={{background:dk?`${navy}`:navy,borderBottom:`${bdw} solid ${dk?"rgba(255,255,255,.08)":"transparent"}`,padding:heroCompact?"10px 24px":"20px 24px",transition:"padding .2s",position:"relative",overflow:"hidden"}}>
          <div style={{maxWidth:isE?780:720,margin:"0 auto",position:"relative"}}>
        ...
        <div style={{maxWidth:isE?780:720,margin:"0 auto",padding:"24px 0"}}>
  ...
  const PreCheckPage = () => {
    ...
    return (
      <div className="an" style={{maxWidth:isE?720:660,margin:"0 auto",padding:isE?"40px 0 60px":"32px 0"}}>
        ...
        {step === "ready" && (
          <div className="td">
            ...
            <Btn primary onClick={() => { setPg("tasks"); setCheckTarget(null); }} style={{width:"100%",justifyContent:"center"}}>
  ...
  const IdpFullScreen = () => {
    ...
    const SettingsGear = () => (
      <div style={{position:"relative"}}>
        <button onClick={()=>setIdpSettingsOpen(!idpSettingsOpen)} style={{width:34,height:34,borderRadius:sr,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:idpSettingsOpen?teal:ts,background:idpSettingsOpen?(dk?"rgba(0,133,117,.12)":"rgba(0,133,117,.06)"):"none",border:`1px solid ${idpSettingsOpen?`${teal}40`:bd}`,transition:"all .15s"}}>
          <I.Gear/>
        </button>
        {idpSettingsOpen && <>
    ...
    // ── Top bar ──
    const TopBar = () => (
      <div style={{height:56,display:"flex",alignItems:"center",padding:"0 20px",gap:14,flexShrink:0,borderBottom:`${bdw} solid ${bd}`,background:dk?"rgba(0,20,60,.5)":"rgba(0,44,119,.03)"}}>
              ...
              <div style={{alignSelf:"flex-start",maxWidth:"90%",display:"flex",flexWrap:"wrap",gap:6,marginTop:4}}>
                {chatQuestions[chatQ].suggestions.map((s,i) => (
                  <button key={i} onClick={()=>handleChatAnswer(s, chatQ)}
                    style={{padding:"8px 14px",borderRadius:20,border:`1px solid ${dk?`${teal}40`:`${teal}30`}`,background:dk?"rgba(0,133,117,.08)":`${teal}06`,color:dk?"#cee8e4":teal,fontSize:12,fontWeight:600,fontFamily:f,cursor:"pointer",transition:"all .15s",lineHeight:1.3}}
                    onMouseEnter={e=>{e.currentTarget.style.background=tealBg;e.currentTarget.style.borderColor=teal;}}
                    onMouseLeave={e=>{e.currentTarget.style.background=dk?"rgba(0,133,117,.08)":`${teal}06`;e.currentTarget.style.borderColor=dk?`${teal}40`:`${teal}30`;}}>
                    {s}
                  </button>
                ))}
              </div>
                ...
                {uploadedFiles.length > 0 && (
                  <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:10}}>
                    {uploadedFiles.map((f2,i) => (
                      <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:ir,background:dk?"rgba(0,133,117,.08)":`${green}08`,border:`1px solid ${green}20`}}>
    ...
    const SummaryStep = () => {
      ...
      return (
        <div className="an" style={{maxWidth:560,margin:"0 auto",padding:"36px 20px"}}>
          ...
          <div style={{background:dk?"rgba(0,133,117,.08)":`${teal}06`,border:`1px solid ${teal}25`,borderRadius:cr,padding:16,marginBottom:24}}>
    ...
    const PlanScreen = () => {
      ...
      return (
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          {/* ── Slim top bar ── */}
          <div style={{display:"flex",alignItems:"center",padding:"0 20px",height:50,gap:10,flexShrink:0,borderBottom:`${bdw} solid ${bd}`,background:dk?"rgba(0,20,60,.3)":"rgba(0,44,119,.02)"}}>
          ...
          <div style={{flex:1,overflow:"auto"}}>
            ...
            {/* ── HERO BANNER ── */}
            <div style={{background:dk?"rgba(0,44,119,.25)":`${navy}10`,borderBottom:`${bdw} solid ${bd}`,padding:"24px 20px 20px"}}>
            ...
            {showReference && (
              <div style={{borderBottom:`${bdw} solid ${bd}`,background:dk?"rgba(0,44,119,.08)":`${navy}03`,padding:"16px 20px"}}>
            ...
            <div style={{padding:"16px 20px 24px"}}>
              <div style={{maxWidth:960,margin:"0 auto"}}>
                ...
                {planEditing && planStatus==="draft" && (
                  <button onClick={()=>setPlanModal("addSkill")} style={{width:"100%",padding:"16px",borderRadius:cr,border:`2px dashed ${dk?"rgba(255,255,255,.1)":"rgba(0,44,119,.1)"}`,background:"none",cursor:"pointer",color:tm,fontSize:14,fontWeight:600,fontFamily:f,display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:12}}>
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:bg,color:tx,fontFamily:f,overflow:"hidden"}}>
      ...
      <nav aria-label="Main navigation" style={{width:mob?260:sbW,height:"100vh",background:sbBg,borderRight:`${bdw} solid ${bd}`,display:"flex",flexDirection:"column",flexShrink:0,position:mob?"fixed":"sticky",top:0,zIndex:mob?1000:2,left:mob?(sbOpen?0:-280):0,transition:"width .2s ease, left .2s ease",overflow:"hidden"}}>
        ...
        <div style={{padding:sbOpen?"8px":"8px 6px",flexShrink:0,borderTop:`1px solid ${dk?"rgba(255,255,255,.05)":"rgba(0,0,0,.04)"}`}}>
          ...
          <button onClick={()=>mob?setMob(false):setSbOpen(!sbOpen)} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,width:"100%",padding:"8px",borderRadius:sr,cursor:"pointer",color:tm,fontSize:12,fontFamily:f,background:"none",border:"none",marginTop:4}}>{sbOpen?<I.Collapse/>:<I.Expand/>}{sbOpen&&<span>Collapse</span>}</button>
        </div>
      </nav>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <main id="main" ref={mainRef} style={{flex:1,padding:"0 24px",overflow:"auto"}}>{renderMain()}</main>
      </div>
