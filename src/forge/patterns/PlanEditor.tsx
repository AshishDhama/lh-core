...
function GoalCard({ goal, onGoalUpdate, onDeleteGoal }: GoalCardProps) {
  ...
  return (
    <div className="flex flex-col gap-0 rounded-2xl border border-border bg-surface-primary overflow-hidden">
      {/* Goal header */}
      <div
      ...
      >
        ...
        {onDeleteGoal && (
          <button
            type="button"
            className="flex-shrink-0 flex items-center justify-center w-7 h-7 rounded-lg hover:bg-error-light/30 transition-colors"
            onClick={(e) => {
              e.stopPropagation();
